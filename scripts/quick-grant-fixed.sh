#!/bin/bash
# Quick grant with transaction_type fix
...
transaction_info AS (
    INSERT INTO payment_transactions (user_id, invoice_number, amount, currency, status, payment_method, transaction_type, created_at, updated_at)
    SELECT 
        user_id, 
