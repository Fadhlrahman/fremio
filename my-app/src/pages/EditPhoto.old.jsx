...
const renderDebugPanel = () => (
  ...
  >
    <h2 style={{ marginTop: 0 }}>Debug: capturedVideos</h2>
    <button
    ...
    <div>
      ...
      <pre style={{ background: "#333", padding: "0.5em", overflowX: "auto" }}>
        {debugCapturedVideos}
      </pre>
    </div>
    <div>
      <strong>Parsed video objects:</strong>
      <pre style={{ background: "#333", padding: "0.5em", overflowX: "auto" }}>
        {JSON.stringify(debugParsedVideos, null, 2)}
      </pre>
    </div>
  </div>
);
...
// ...existing JSX...
// At the top level of your return statement:
...
const FILTER_PRESETS = [
  {
    id: "none",
    label: "Original",
    description: "Tanpa filter, warna asli foto",
    css: "",
  },
  {
    id: "warm",
    label: "Warm Glow",
    description: "Tona hangat dan sedikit saturasi",
    css: "sepia(0.15) saturate(1.2) hue-rotate(-5deg) contrast(1.05)",
  },
  {
    id: "mono",
    label: "Monokrom",
    description: "Hitam putih klasik",
    css: "grayscale(1) contrast(1.1)",
  },
  {
    id: "vintage",
    label: "Vintage",
    description: "Nuansa retro lembut",
    css: "sepia(0.4) saturate(0.9) brightness(1.05) contrast(0.95)",
  },
  {
    id: "cool",
    label: "Cool Breeze",
    description: "Nada dingin dan tajam",
    css: "hue-rotate(15deg) saturate(1.15) contrast(1.05)",
  },
  {
    id: "vivid",
    label: "Vivid",
    description: "Warna lebih hidup dan kontras",
    css: "saturate(1.35) contrast(1.15)",
  },
  {
    id: "soft",
    label: "Soft Pastel",
    description: "Lembut dengan warna pastel",
    css: "saturate(0.85) brightness(1.1)",
  },
  {
    id: "dramatic",
    label: "Dramatis",
    description: "Kontras tinggi dengan bayangan tegas",
    css: "contrast(1.3) brightness(0.95) saturate(1.05)",
  },
];
...
export default function EditPhoto() {
  console.log('🚀 EDITPHOTO COMPONENT RENDERING...');
  
  // Wrap entire component in try-catch
  try {
    return <EditPhotoInner />;
  } catch (error) {
    console.error('❌ EDITPHOTO RENDER ERROR:', error);
    return (
      <div style={{
        minHeight: '100vh',
        display: 'flex',
        ...
        <div style={{
          ...
          <h2 style={{ color: 'red', marginBottom: '1rem' }}>❌ EditPhoto Render Error</h2>
          <pre style={{ whiteSpace: 'pre-wrap', fontSize: '14px', background: '#f5f5f5', padding: '1rem', borderRadius: '6px' }}>
            {error.toString()}
            {'\n\n'}
            {error.stack}
          </pre>
...
function EditPhotoInner() {
  ...
  const [designerElements, setDesignerElements] = useState([]); // elements from custom frame designer (text, shapes, uploads)
  ...
  const [devAuthStatus, setDevAuthStatus] = useState({
    loading: false,
    error: null,
    success: false,
  });
  ...
  const [debugCapturedVideos, setDebugCapturedVideos] = useState("");
  const [debugParsedVideos, setDebugParsedVideos] = useState([]);
  ...
  const [shouldAutoSelectInitialSlot, setShouldAutoSelectInitialSlot] =
    useState(() => {
  ...
  const assignSlotElement = useCallback(
    (slotIndex, element) => {
  ...
  const canvasFilterSupported = useMemo(
    () => (isBrowser ? detectCanvasFilterSupport() : false),
    [isBrowser]
  );
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn(
        "⚠️ Failed to reset developer access flag from sessionStorage.",
        error
      );
    } finally {
  ...
  const pagePadding = useMemo(() => {
    ...
    const topPadding = `calc(env(safe-area-inset-top, 0px) + ${
      isCompact ? "0.75rem" : "1rem"
    })`;
    const sidePadding = "clamp(0.75rem, 5vw, 1.5rem)";
    const bottomPadding = `calc(env(safe-area-inset-bottom, 0px) + ${
  ...
  const allSlotsShareFilter = (filterId) => {
    ...
    return frameConfig.slots.every(
      (_, index) => resolveSlotFilterId(index) === filterId
    );
  };

  const anySlotHasFilter = () => {
    if (!frameConfig?.slots) return false;
    return frameConfig.slots.some(
      (_, index) => resolveSlotFilterId(index) !== "none"
    );
  };

  const drawImageWithFilter = (
    ctx,
    image,
    dx,
    dy,
    ...
    const { filterId: filterIdOverride, filterCss: filterCssOverride } =
      options;
    const effectiveFilterId = filterIdOverride ?? "none";
    const cssFilter =
      filterCssOverride ??
      (effectiveFilterId === "none"
        ? "none"
        : getFilterCssValue(effectiveFilterId));
  ...
  const getPhotoSourceForSlot = (slotIndex) => {
    if (!frameConfig || slotIndex === null || slotIndex === undefined)
      return null;
    ...
    if (frameConfig?.duplicatePhotos) {
      ...
      if (duplicateSlot) {
        const photoIndex =
          duplicateSlot.photoIndex !== undefined
            ? duplicateSlot.photoIndex
            : slotIndex;
    ...
    if (!slot) return null;
    const photoIndex =
      slot.photoIndex !== undefined ? slot.photoIndex : slotIndex;
    return photos[photoIndex] ?? null;
  };
  ...
  const getVideoSourceForSlot = (slotIndex) => {
    ...
    if (resolvedByPhoto) {
      return {
        ...resolvedByPhoto,
        baseVideoIndex:
          resolvedByPhoto.baseVideoIndex ??
          (slot.photoIndex !== undefined ? slot.photoIndex : slotIndex),
      };
    }

    const slotOverride = slotVideos[slotIndex];
    if (slotOverride && (slotOverride.dataUrl || slotOverride.blob)) {
      const baseVideoIndex =
        slotOverride.baseVideoIndex ??
        (slot.photoIndex !== undefined ? slot.photoIndex : slotIndex);
      return {
        ...slotOverride,
        baseVideoIndex,
    ...
    if (frameConfig?.duplicatePhotos) {
      ...
      if (sharedOverride && (sharedOverride.dataUrl || sharedOverride.blob)) {
    ...
    if (fallbackVideo && (fallbackVideo.dataUrl || fallbackVideo.blob)) {
      return {
        ...fallbackVideo,
      ...
      if (!clicked) {
        ...
        reader.onloadend = () => {
          ...
          if (typeof dataUrl === "string") {
            const opened = triggerAnchorClick(dataUrl, {
              target: "_blank",
              rel: "noopener",
            });
        ...
        reader.onerror = (error) => {
          console.warn(
            "⚠️ Failed to convert blob to data URL for iOS download fallback:",
            error
          );
          window.location.href = url;
        };
  ...
  const downloadMediaBundle = async ({
    ...
    try {
      ...
      if (framedVideoResult?.blob) {
        videosFolder.file(framedVideoResult.filename, framedVideoResult.blob, {
          binary: true,
        });
        videosAdded += 1;
      ...
      if (!framedVideoResult?.blob) {
        ...
        for (const { index, clip } of recordedClips) {
          ...
          if (!sourceBlob) {
            console.warn(
              `⚠️ Gagal menyiapkan blob video dari slot ${
                index + 1
              }, melewati fallback.`
            );
            continue;
          ...
          const clipMime = (() => {
            if (
              ...
              clip.mimeType.includes("/")
            ) {
              return clip.mimeType;
            }
            if (
              typeof sourceBlob.type === "string" &&
              sourceBlob.type.includes("/")
            ) {
          ...
          const rawFilename = `raw-slot-${
            index + 1
          }-${timestampPart}.${extension}`;
          rawClips.push({
        ...
        if (rawClips.length) {
          ...
          rawClips.forEach(
            ({ filename, blob, slotIndex, mimeType, duration }) => {
      ...
      if (!videosAdded) {
        videosFolder.file(
          ...
          `Tidak ada video yang tersedia untuk sesi ini.\nAlasan: ${
            framedVideoResult?.reason ?? "video-belum-dibuat"
        ...
        const timer = setTimeout(() => {
          if (generationController) {
            try {
              ...
              console.warn(
                "⚠️ Failed to abort ZIP generation after timeout:",
                abortError
              );
            }
          }
      ...
      console.error(
        "❌ Failed to create media bundle, falling back to individual downloads:",
        bundleError
      );
  ...
  const downloadRecordedCountdownClips = async (timestampSuffix = "") => {
    ...
    const safeTimestamp =
      timestampSuffix || new Date().toISOString().replace(/[:.]/g, "-");
    ...
    for (const { index, clip } of recordedClips) {
      ...
      if (!blob) {
        console.warn(
          `⚠️ Skipping countdown clip ${index + 1}: failed to create blob`
        );
        continue;
      }

      const baseFilename = `fremio-countdown-${safeTimestamp}-slot-${
        index + 1
      }`;
      ...
      if (!blob.type.includes("mp4")) {
        try {
          ...
          if (mp4Blob) {
            ...
            finalExtension =
              clip?.mimeType && clip.mimeType.includes("/")
          ...
          console.error(
            `❌ Failed to convert countdown clip ${index + 1} to MP4:`,
            conversionError
          );
          finalExtension =
            clip?.mimeType && clip.mimeType.includes("/")
  ...
  const createRecorderFromStream = (stream, options = {}) => {
    ...
    for (const mimeType of preferredTypes) {
      try {
        ...
        console.warn(
          `⚠️ Failed to initialize MediaRecorder with ${mimeType}:`,
          error
        );
      }
    }
    ...
    if (!recorder) {
      try {
        ...
        console.error(
          "❌ Failed to initialize MediaRecorder with default options:",
          error
        );
        return null;
      }
  ...
  const handleDeveloperUnlock = async (event) => {
    ...
    if (!trimmedToken) {
      setDevAuthStatus({
        loading: false,
        error: "Masukkan token developer terlebih dahulu.",
        success: false,
      });
      return;
    }
  ...
  const calculateSmartDefaultScale = (photoImg, slotIndex, frameConfig) => {
    if (!frameConfig || !frameConfig.slots[slotIndex] || !photoImg) {
      console.log(
        "⚠️ Missing data for smart scale calculation, using fallback scale 1.6"
      );
      return 1.6;
    }
    ...
    console.log(
      `🧮 Smart Scale Calculation for Slot ${slotIndex + 1} (${slot.id}):`
    );
    console.log(
      `  - Photo: ${photoImg.width}x${
        photoImg.height
      } (ratio: ${photoAspectRatio.toFixed(2)})`
    );
    console.log(
      `  - Slot: ${slot.width}x${slot.height} (ratio: ${(
    ...
    console.log(
      `  - Slot in pixels: ${slotWidthPx.toFixed(1)}x${slotHeightPx.toFixed(
        1
      )}px`
    );
    ...
    console.log(
      `  - Contained photo size: ${containedPhotoWidth.toFixed(
        1
      )}x${containedPhotoHeight.toFixed(1)}px`
    );
    ...
    const heightScale =
      containedPhotoHeight < slotHeightPx
        ? slotHeightPx / containedPhotoHeight
        : 1;

    // Step 4: Check width constraint
    // If contained photo width < slot width, we need to scale up
    const widthScale =
      containedPhotoWidth < slotWidthPx ? slotWidthPx / containedPhotoWidth : 1;
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (savedVideos) {
          try {
            ...
            console.warn(
              "⚠️ Failed to parse capturedVideos from localStorage:",
              videoParseError
            );
          }
        }
    ...
    const frameConfigForDefaults = resolvedConfig;
        ...
        if (
          frameConfigForDefaults?.duplicatePhotos &&
          frameConfigForDefaults.slots
        ) {
          ...
          frameConfigForDefaults.slots.forEach((slot, slotIndex) => {
            ...
            console.log(
              `📐 Setting slot ${slotIndex} (photo ${photoIndex}) default scale: ${defaultScale}`
            );
          ...
          console.log(
            "🎯 Initialized slot photos for duplicate frame:",
            Object.keys(initialSlotPhotos).length,
            "slots"
          );
        } else {
    ...
    if (frameFromStorage === "Testframe3") {
      ...
      console.log(
        "  - frameConfig from localStorage:",
        safeStorage.getItem("frameConfig")
      );
      console.log(
        "  - capturedPhotos from localStorage:",
        safeStorage.getItem("capturedPhotos")
      );
    }
  ...
  const config = resolvedConfig;
    ...
    if (config) {
      ...
      // ✅ NEW APPROACH: Convert photo slots to designer elements!
      // Photo slots (type='photo') will be treated as upload elements with placeholder
      // This allows unified z-index layering with text/shapes
      const photoElements = Array.isArray(config?.designer?.elements)
        ? config.designer.elements.filter(el => el?.type === 'photo')
        : [];
      
      // Convert photo slots to pseudo-upload elements for rendering
      const photoAsUploadElements = photoElements.map((photoEl, idx) => ({
        ...photoEl,
        type: 'upload', // ✅ Treat as upload for rendering
        data: {
          ...photoEl.data,
          image: null, // Will be filled from capturedPhotos
          photoIndex: photoEl.data?.photoIndex ?? idx,
          borderRadius: photoEl.data?.borderRadius || 24,
          objectFit: 'cover',
        }
      }));
      
      // Load other designer elements (text, shapes, real uploads - NOT photo slots, NOT background)
      const otherDesignerElems = Array.isArray(config?.designer?.elements) 
        ? config.designer.elements.filter(el => 
            el && 
            el.type !== 'photo' && 
      ...
      // ✅ COMBINE: photo slots (now as uploads) + other designer elements
      const allDesignerElements = [...photoAsUploadElements, ...otherDesignerElems];
      
      setDesignerElements(allDesignerElements);
      console.log('🎨 Loaded designer elements (including photo slots as uploads):', {
        total: allDesignerElements.length,
        photoSlots: photoAsUploadElements.length,
        other: otherDesignerElems.length
      });
      
      // Load background photo element for custom frames
      const backgroundPhoto = Array.isArray(config?.designer?.elements)
        ? config.designer.elements.find(el => el?.type === 'background-photo')
        : null;
      setBackgroundPhotoElement(backgroundPhoto);
      if (backgroundPhoto) {
        console.log('🖼️ Loaded background photo element');
      }
      ...
      console.log("✅ Frame config loaded (final):", {
        ...
        designerElements: designerElems.length,
        hasBackgroundPhoto: !!backgroundPhoto
      });

      // Extra verification for Testframe3
      if (frameFromStorage === "Testframe3") {
        console.log(
          `✅ ${frameFromStorage.toUpperCase()} successfully loaded:`
        );
      ...
      console.error(
        "❌ Failed to resolve frame config after fallbacks for:",
        frameFromStorage
      );
  ...
  // ✅ NEW: Update photo elements with real captured photos
  useEffect(() => {
    if (!photos || photos.length === 0) return;
    if (!designerElements || designerElements.length === 0) return;
    
    // Find photo elements (converted uploads with photoIndex)
    const hasPhotoElements = designerElements.some(el => 
      el.type === 'upload' && typeof el.data?.photoIndex === 'number'
    );
    
    if (!hasPhotoElements) return;
    
    console.log('🔄 Updating photo elements with captured photos...');
    
    // Update photo elements with real images
    const updatedElements = designerElements.map(el => {
      if (el.type === 'upload' && typeof el.data?.photoIndex === 'number') {
        const photoIndex = el.data.photoIndex;
        const photoSrc = photos[photoIndex];
        
        if (photoSrc && (!el.data.image || el.data.image !== photoSrc)) {
          console.log(`📷 Updated photo element ${photoIndex} with real image`);
          return {
            ...el,
            data: {
              ...el.data,
              image: photoSrc, // ✅ Fill with real captured photo
            }
          };
        }
      }
  ...
  }, [photos]); // Only depend on photos, not designerElements to avoid loop
  ...
  useEffect(() => {
    ...
    setPhotoFilters((prev = {}) => {
      ...
      frameConfig.slots.forEach((_, index) => {
        const existing = Object.prototype.hasOwnProperty.call(prev, index)
          ? prev[index]
          : "none";
        next[index] = existing;
  ...
  useEffect(() => {
    ...
    frameConfig?.id,
    frameConfig?.slots?.length,
    frameConfig?.duplicatePhotos,
    photos,
    slotPhotos,
    swapModeActive,
    selectedPhotoForEdit,
    shouldAutoSelectInitialSlot,
  ]);
  ...
  useEffect(() => {
    const checkFrameChange = () => {
      ...
      if (
        ...
        currentFrameFromStorage &&
        currentFrameFromStorage !== selectedFrame
      ) {
        console.log(
  ...
  const performSlotSwap = async (sourceSlotIndex, targetSlotIndex) => {
    ...
    if (frameConfig.duplicatePhotos) {
      ...
      const srcImage = Object.prototype.hasOwnProperty.call(
        newSlotPhotos,
        sourceSlotIndex
      )
        ? newSlotPhotos[sourceSlotIndex]
        : photos[srcBaseIndex] ?? null;
      const dstImage = Object.prototype.hasOwnProperty.call(
        newSlotPhotos,
        targetSlotIndex
      )
      ...
      const srcVideo = Object.prototype.hasOwnProperty.call(
        newSlotVideos,
        sourceSlotIndex
      )
        ? newSlotVideos[sourceSlotIndex]
        : videos[srcBaseIndex] ?? null;
      const dstVideo = Object.prototype.hasOwnProperty.call(
        newSlotVideos,
        targetSlotIndex
      )
      ...
      // Synchronize videos array and capturedVideos for export
      const updatedVideos = [...videos];
      Object.keys(newSlotVideos).forEach((slotIdx) => {
        const idx = Number(slotIdx);
        if (!Number.isNaN(idx)) {
          updatedVideos[idx] = newSlotVideos[slotIdx] || null;
        }
      });
      setVideos(updatedVideos);
      safeStorage.setItem("capturedVideos", JSON.stringify(updatedVideos));
      ...
      setTimeout(async () => {
        ...
        for (const slotIdx of affectedSlots) {
          ...
          try {
            const smartScale = await calculateSmartScaleAsync(
              newPhotoForSlot,
              slotIdx
            );
            ...
            console.log(
              `✨ Updated smart scale for slot ${
                slotIdx + 1
              }: ${smartScale.toFixed(2)}x`
            );
          } catch (error) {
            console.error(
              `❌ Failed to update smart scale for slot ${slotIdx + 1}:`,
              error
            );
          }
        }
      ...
      console.log(
        `🔄 Swapped duplicate slots ${sourceSlotIndex + 1} ↔ ${
          targetSlotIndex + 1
        }`
      );
      return true;
    ...
    const sourceSlotHasVideo = Object.prototype.hasOwnProperty.call(
      newSlotVideos,
      sourceSlotIndex
    );
    const targetSlotHasVideo = Object.prototype.hasOwnProperty.call(
      newSlotVideos,
      targetSlotIndex
    );
    ...
    console.log(
      `🔄 Swapped slots ${sourceSlotIndex + 1} ↔ ${targetSlotIndex + 1}`
    );
    return true;
  };
  ...
  const resolveSlotPhotoDimensions = useCallback(
    (slotIndex) => {
      if (
        photoDimensions?.[slotIndex]?.width &&
        photoDimensions?.[slotIndex]?.height
      ) {
  ...
  const getSlotPixelDimensions = useCallback(
    (slotIndex) => {
      const slot = frameConfig?.slots?.[slotIndex];
      if (!slot) return null;
      const measurement = slotMeasurements?.[slotIndex] || null;
      const width =
        measurement?.width ?? slot.width * Math.max(previewWidth, 1);
      const height =
        measurement?.height ?? slot.height * Math.max(previewHeight, 1);
      return { width, height };
    },
    [frameConfig?.slots, slotMeasurements, previewWidth, previewHeight]
  );

  const getSlotPanMetrics = useCallback(
    (slotIndex) => {
      const slot = frameConfig?.slots?.[slotIndex];
      if (!slot) return null;
      ...
      const slotPixels = getSlotPixelDimensions(slotIndex);
      const slotWidthPx =
        slotPixels?.width ?? slot.width * Math.max(previewWidth, 1);
      const slotHeightPx =
        slotPixels?.height ?? slot.height * Math.max(previewHeight, 1);

      if (
        !Number.isFinite(slotWidthPx) ||
        slotWidthPx <= 0 ||
        !Number.isFinite(slotHeightPx) ||
        slotHeightPx <= 0
      ) {
        return null;
      }
    ...
    [
      frameConfig?.id,
      frameConfig?.slots,
      getSlotPixelDimensions,
      previewHeight,
      previewWidth,
      resolveSlotPhotoDimensions,
    ]
  );

  const calculateCoverageScaleForSlot = useCallback(
    (slotIndex) => {
      ...
      const { slotWidthPx, slotHeightPx, containedWidthPx, containedHeightPx } =
        metrics;
      ...
      const heightScale = containedHeightPx
        ? slotHeightPx / containedHeightPx
        : 1;
      ...
      console.log(
        `🔍 Coverage scale for slot ${slotIndex + 1}: ${clamped.toFixed(
          2
        )}x (height: ${heightScale.toFixed(2)}x, width: ${widthScale.toFixed(
          2
        )}x)`
      );
      return clamped;
  ...
  const adjustPanForEdgeBoundaries = useCallback(
    (current, proposedScale, slotIndex) => {
      const baseTransform =
      ...
      const scale =
        typeof proposedScale === "number" &&
        Number.isFinite(proposedScale) &&
        proposedScale > 0
          ? proposedScale
          : typeof baseTransform?.scale === "number" &&
            Number.isFinite(baseTransform.scale) &&
      ...
      const maxTranslateX =
        maxActualTranslateX > 0 ? maxActualTranslateX / safeScale : 0;
      const maxTranslateY =
        maxActualTranslateY > 0 ? maxActualTranslateY / safeScale : 0;
  ...
  const calculateMaxZoomOutScale = (slotIndex) =>
    calculateCoverageScaleForSlot(slotIndex);
  ...
  const calculateAutoFillScale = (slotIndex) => {
    ...
    if (photoImg) {
      ...
      const smartScale = calculateSmartDefaultScale(
        photoImg,
        slotIndex,
        frameConfig
      );
    ...
    if (frameConfig?.id === "Testframe4") {
      ...
      console.log(
        `🎯 Testframe4 slot ${
          slotIndex + 1
        }: Setting to MAX ZOOM OUT + 6 zoom in steps`
      );
      ...
      console.log(
        `📏 Testframe4 Slot ${
          slotIndex + 1
        }: Default scale = ${adjustedScale.toFixed(
          2
        )}x (min coverage: ${coverageScale.toFixed(2)}x)`
      );
  ...
  const generateFramedVideo = async ({
    ...
    if (!isBrowser) {
      console.log(
        "🎬 Skipping framed video generation outside browser environment"
      );
    ...
    if (!frameConfig?.slots || frameConfig.slots.length === 0) {
      console.warn(
        "🎬 Frame configuration missing slots; skipping framed video generation"
      );
      return { success: false, reason: "no-frame-slots" };
    }
    console.log("🎬 Frame config has", frameConfig.slots.length, "slots");

    const slotVideoInfos = frameConfig.slots.map((_, index) =>
      getVideoSourceForSlot(index)
    );
    console.log(
      "🎬 Slot video infos:",
      slotVideoInfos.map((info, i) => ({
        slot: i,
    ...
    if (!hasVideo) {
      console.log(
        "🎬 No recorded videos available for framing. Skipping video export."
      );
    ...
    const slotPhotoElements = await Promise.all(
      frameConfig.slots.map(async (slot, slotIndex) => {
        let photoData;
        if (frameConfig.duplicatePhotos) {
        ...
        video.onloadedmetadata = () => {
          if (video.readyState >= 1) {
            try {
              ...
              console.warn(
                "⚠️ Unable to reset recorded video timeline:",
                resetError
              );
            }
          }
    ...
    for (let i = 0; i < slotVideoInfos.length; i++) {
      ...
      // Accept if info has a valid dataUrl or blob
      if (!info || (!info.dataUrl && !info.blob)) {
        console.warn(`⚠️ No dataUrl/blob for slot ${i + 1}; skipping`);
    ...
    console.log(
      `🎬 Max video duration: ${DEFAULT_TARGET_DURATION}s (${
    ...
    const getTargetDuration = (entry) => {
      ...
      const infoDuration = clampDuration(
        entry.info?.duration,
      ...
      const mediaDuration = clampDuration(
    ...
    const maxDuration = slotTargetDurations.reduce(
      (max, value) => Math.max(max, value),
      0
    );
    if (!maxDuration) {
      console.warn(
        "🎬 Recorded videos report zero duration; skipping framed video export"
      );
    ...
    const calculateDefaultVideoScale = ({
      ...
      const currentDisplayDimension = isPortraitVideo
        ? displayWidth
        : displayHeight;
    ...
    const drawMediaInSlot = (media, slotIndex, options = {}) => {
      ...
      const resolvedFilterId =
        filterIdOverride ?? resolveSlotFilterId(slotIndex);
      const slotFilterCss = (() => {
    ...
    const encodeWithFfmpegFallback = async () => {
      ...
      try {
        ...
        const fallbackFrameRate = Math.max(
          12,
          Math.min(21, Math.round(frameRate * 0.8))
        );
        const totalDuration = maxDuration + 0.25;
        const padDigits = Math.max(
          4,
          String(Math.ceil(totalDuration * fallbackFrameRate) + 2).length
        );
        ...
        cleanupArtifacts = async () => {
          await Promise.all(
            capturedFrameNames.map(async (name) => {
              try {
        ...
        await Promise.all(
          activeVideos.map(async ({ video }) => {
            try {
              ...
              console.warn(
                "⚠️ FFmpeg fallback: video playback start issue",
                error
              );
            }
          })
        ...
        while (true) {
          ...
          const frameName = `${framePrefix}_${String(framesCaptured).padStart(
            padDigits,
            "0"
          )}.jpg`;
        ...
        activeVideos.forEach(({ video }) => {
          try {
            ...
            console.warn(
              "⚠️ FFmpeg fallback: failed to pause video",
              pauseError
            );
          }
        });
    ...
    console.log(
      `🎬 Video settings: ${videoBitrate / 1_000_000}Mbps @ ${frameRate}fps (${
        mobileDevice ? "mobile" : "desktop"
      })`
    );

    const stream = videoCanvas.captureStream(frameRate);
    const recorder = createRecorderFromStream(stream, {
      videoBitsPerSecond: videoBitrate,
    });
    if (!recorder) {
      console.warn(
        "🎬 Unable to initialize MediaRecorder; switching to FFmpeg fallback"
      );
    ...
    const recordingPromise = new Promise((resolve, reject) => {
      ...
      recorder.onerror = (event) => {
        console.error(
          "❌ MediaRecorder error during framed video creation:",
          event
        );
    ...
    await Promise.all(
      activeVideos.map(async ({ video }) => {
        try {
          ...
          console.warn(
            "⚠️ Video playback interrupted during framed video render:",
            error
          );
        }
      })
      ...
      slotVideoElements.forEach((entry) => {
        if (entry?.video) {
          try {
            ...
            console.warn(
              "⚠️ Failed to pause video during cleanup:",
              pauseError
            );
          }
        }
    ...
    try {
      ...
      console.error(
        "❌ Failed to finalize framed video recording:",
        recordingError
      );
      stopRecording();
      ...
      return (
        fallbackResult || {
          success: false,
          reason: "recording-finalize-failed",
    ...
    if (!videoBlob.type.includes("mp4") && !mobileDevice) {
      try {
        ...
        if (mp4Blob) {
          ...
          console.warn(
            "⚠️ MP4 conversion failed; falling back to original WebM download."
          );
        }
      } catch (conversionError) {
        console.error(
          "❌ MP4 conversion threw an error; falling back to WebM download.",
          conversionError
        );
      }
    } else if (mobileDevice) {
      console.log(
        "📱 Mobile device detected - skipping MP4 conversion, using WebM directly"
      );
    }
  ...
  const getPhotoImageForSlot = (slotIndex) => {
    try {
      ...
      console.log(
        "⚠️ Could not get photo image for smart scale calculation:",
        error
      );
      return null;
    }
  ...
  const calculateSmartScaleAsync = async (photoDataUrl, slotIndex) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        console.log(
          `✨ Async smart scale for slot ${slotIndex + 1}: ${smartScale.toFixed(
            2
          )}x`
        );
        resolve(smartScale);
  ...
  const updateSmartScales = async () => {
    ...
    if (frameConfig.duplicatePhotos && frameConfig.slots) {
      for (
        let slotIndex = 0;
        slotIndex < frameConfig.slots.length;
        slotIndex++
      ) {
        const slot = frameConfig.slots[slotIndex];
        ...
        if (photoForSlot) {
          try {
            ...
            const existingTransform = photoTransforms[slotIndex] || {
              translateX: 0,
              translateY: 0,
            };
            ...
            console.log(
              `✨ Smart scale for slot ${slotIndex + 1}: ${smartScale.toFixed(
                2
              )}x`
            );
          } catch (error) {
            console.warn(
              `⚠️ Failed to calculate smart scale for slot ${slotIndex + 1}:`,
              error
            );
          }
        }
      ...
      for (let i = 0; i < photos.length; i++) {
        try {
          ...
          // Keep existing transforms if they exist, only update scale when user hasn't adjusted manually
          const existingTransform = photoTransforms[i] || {
            translateX: 0,
            translateY: 0,
          };
          const wasUserAdjusted = Boolean(existingTransform.userAdjusted);
          ...
          console.error(
            `❌ Error calculating smart scale for photo ${i + 1}:`,
            error
          );
        }
      }
  ...
  useEffect(() => {
    ...
    const entries = Object.entries(slotPhotos).filter(([, value]) =>
      Boolean(value)
    );
      ...
      for (const [slotKey, photoDataUrl] of entries) {
        ...
        try {
          ...
          setPhotoTransforms((prev = {}) => {
            ...
            const targetScale =
              wasUserAdjusted && typeof previous.scale === "number"
                ? previous.scale
                : requiredScale;
          ...
          console.warn(
            `⚠️ Failed to update transforms for slot ${slotIndex + 1}:`,
            error
          );
        }
      }
    ...
    frameConfig?.id,
    slotPhotos,
    calculateCoverageScaleForSlot,
    adjustPanForEdgeBoundaries,
  ]);
  ...
  const calculateAutoFillScale_ORIGINAL = (slotIndex) => {
    ...
    if (frameConfig?.id === "Testframe4") {
      ...
      console.log(
        `📏 Testframe4 Slot ${
          slotIndex + 1
        }: Default scale = ${defaultScale.toFixed(
          2
        )}x (max zoom out: ${maxZoomOutScale.toFixed(
          2
        )}x + ${zoomInSteps} steps)`
      );
      return defaultScale;
    ...
    console.log(
      `📏 Slot ${slotIndex + 1}: Vertical-fit scale = ${clampedScale.toFixed(
        2
      )}x (slot: ${(slot.width * 100).toFixed(0)}%×${(
        slot.height * 100
      ).toFixed(0)}%)`
    );
  ...
  const initializePhotoScale = async (photoIndex) => {
    if (!photos[photoIndex] || !frameConfig) {
      console.warn(
        `⚠️ Cannot initialize photo scale for slot ${
          photoIndex + 1
        }: missing photo or frame config`
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `🔧 Initializing photo ${
          photoIndex + 1
        } with smart scale: ${smartScale.toFixed(2)}x`
      );
      ...
      console.error(
        `❌ Error initializing smart scale for photo ${photoIndex + 1}:`,
        error
      );
      ...
      console.log(
        `🔧 Fallback: Initializing photo ${
          photoIndex + 1
        } with auto-fit scale: ${autoFitScale.toFixed(2)}x`
      );
  ...
  const calculatePhotoCropStyle = (frameConfig, slotIndex) => {
    ...
    const transform = photoTransforms[slotIndex] || {
      scale: 1,
      translateX: 0,
      translateY: 0,
    };
    ...
    const scale =
      Number.isFinite(transform?.scale) && transform.scale > 0
        ? transform.scale
        : 1;
    const translateX = Number.isFinite(transform?.translateX)
      ? transform.translateX
      : 0;
    const translateY = Number.isFinite(transform?.translateY)
  ...
  const frameId = resolveStoredFrameId();
      ...
      if (newConfig) {
        ...
        console.warn(
          "⚠️ Manager reload returned null, using fallback getFrameConfig"
        );
  ...
  const handlePhotoZoom = (photoIndex, delta) => {
    setPhotoTransforms((prev) => {
      ...
      const adjustedTransform = adjustPanForEdgeBoundaries(
      ...
      console.log(
        `🔍 Photo ${photoIndex + 1}: Zoom ${
          delta > 0 ? "IN" : "OUT"
        } to ${newScale.toFixed(2)}x (min allowed: ${minimumScale.toFixed(2)}x)`
      );
  ...
  const handlePhotoPan = (photoIndex, deltaX, deltaY) => {
    ...
    setPhotoTransforms((prev) => {
      ...
      const boundedTransform = adjustPanForEdgeBoundaries(
        candidateTransform,
        scale,
        photoIndex
      );
  ...
  const resetPhotoTransform = (photoIndex) => {
    ...
    console.log(
      `🔄 Reset Photo ${photoIndex + 1} to auto-fit: ${autoFillScale.toFixed(
        2
      )}x with centered position`
    );
  ...
  const handlePhotoMouseDown = (e, photoIndex) => {
    ...
    if (
      selectedPhotoForEdit === photoIndex &&
  ...
  const calculateSlotPixels = (frameConfig, slotIndex) => {
    ...
    return {
      ...
      calculatedRatio:
        (slot.width * FRAME_WIDTH) / (slot.height * FRAME_HEIGHT),
    };
  };
  ...
  const savePhotoOnly = async () => {
    ...
    console.log("📊 State check:", {
      ...
      photosPreview: photos.map((p, i) => ({
        index: i,
        hasData: !!p,
        length: p?.length,
        prefix: p?.substring(0, 30),
      })),
      slotPhotosKeys: Object.keys(slotPhotos),
      slotPhotosPreview: Object.entries(slotPhotos).map(([k, v]) => ({
        slot: k,
        hasData: !!v,
        length: v?.length,
        prefix: v?.substring(0, 30),
      })),
      frameConfigSlots: frameConfig?.slots?.length,
      frameConfigSlotsDetail: frameConfig?.slots?.map((s, i) => ({
        index: i,
    ...
    const resolveTransformForSlot = (slotIndex, slot) => {
      ...
      const photoIndex = Number.isInteger(slot?.photoIndex)
        ? slot.photoIndex
        : slotIndex;
    ...
    for (let slotIndex = 0; slotIndex < frameConfig.slots.length; slotIndex++) {
      ...
      console.log(
        `🎯 Slot ${slotIndex}: photoForSlot=${!!photoForSlot}, slotPhotos=${!!slotPhotos[
          slotIndex
        ]}, photos[${slot.photoIndex}]=${!!photos[slot.photoIndex]}`
      );
      ...
      // Validate photo data
      if (
        typeof photoForSlot !== "string" ||
        !photoForSlot.startsWith("data:image/")
      ) {
        console.error(
          `❌ Invalid photo data for slot ${slotIndex}:`,
          photoForSlot?.substring(0, 50)
        );
        continue;
      }
...

      {
        ...
        const photoAspectRatio =
          img.naturalWidth / Math.max(img.naturalHeight, 1);
        ...
        const metrics =
          typeof getSlotPanMetrics === "function"
            ? getSlotPanMetrics(slotIndex)
            : null;
        const previewBaseWidth =
          metrics?.containedWidthPx ?? metrics?.slotWidthPx ?? basePhotoWidth;
        const previewBaseHeight =
          metrics?.containedHeightPx ??
          metrics?.slotHeightPx ??
          basePhotoHeight;

        const transform = resolveTransformForSlot(slotIndex, slot) || {
          scale: 1.0,
          translateX: 0,
          translateY: 0,
        };
        ...
        const translateRatioX = previewBaseWidth
          ? translateX / previewBaseWidth
          : 0;
        const translateRatioY = previewBaseHeight
          ? translateY / previewBaseHeight
          : 0;
        ...
        const finalPhotoX =
          slotCenterX - finalPhotoWidth / 2 + baseTranslateX * scale;
        const finalPhotoY =
          slotCenterY - finalPhotoHeight / 2 + baseTranslateY * scale;
        ...
        console.log(
          `🎨 Drawing photo for slot ${slotIndex} at (${Math.round(
            finalPhotoX
          )}, ${Math.round(finalPhotoY)}) with size ${Math.round(
            finalPhotoWidth
          )}x${Math.round(finalPhotoHeight)}, scale=${scale.toFixed(
            2
          )}, filter=${slotFilterId}`
        );
        ...
        try {
          console.log(
            `🖼️ Attempting direct drawImage for slot ${slotIndex}...`
          );
          ctx.drawImage(
            img,
            finalPhotoX,
          ...
          if (slotFilterId && slotFilterId !== "none") {
            console.log(
              `🎨 Applying filter ${slotFilterId} for slot ${slotIndex}...`
            );
          ...
          console.error(
            `❌ Error drawing image for slot ${slotIndex}:`,
            err,
            err.stack
          );
        }
  ...
  const saveVideoOnly = async () => {
    ...
    if (!hasVideosFromCamera) {
      ...
      alert(
        "No video available. Videos are only available when using the camera."
      );
      setTimeout(() => setSaveStatusMessage(""), 3000);
      return;
    }
    ...
    console.log(
      "✅ [3/6] Photos loaded:",
      loadedPhotos.filter((p) => p.img).length,
      "of",
      photos.length
    );
    ...
    if (framedVideoResult && framedVideoResult.success) {
      ...
      framedVideoResult &&
      framedVideoResult.reason === "recorder-init-failed"
    ) {
      console.error(
        "❌ Video generation failed due to recorder init issue:",
        framedVideoResult
      );
      setSaveStatusMessage(
        "❌ Perangkat tidak mendukung perekaman kanvas otomatis."
      );
      alert(
        "Perangkat ini belum mendukung perekaman video dengan frame secara langsung. Silakan gunakan desktop atau peramban yang lebih baru."
      );
    } else {
  ...
  const handleSaveVideo = async () => {
    ...
    try {
      ...
      console.log(
        `⏱️ Video generation timeout set to ${timeoutMs / 1000}s (${
          mobileDevice ? "mobile" : "desktop"
        })`
      );
      ...
      if (error.message === "Video generation timeout") {
        setSaveStatusMessage(
          "⏱️ Rendering melebihi batas waktu. Coba kurangi durasi video."
        );
        alert(
          "Video generation took too long. Please try with shorter videos or fewer photos."
        );
      } else {
    ...
    try {
      ...
      const loadBasePhoto = (photoDataUrl, index) =>
        new Promise((resolve) => {
          ...
          img.onload = () => {
            console.log(
              `✅ Photo ${index + 1}: Loaded (${img.width}x${img.height})`
            );
            resolve({ img, index });
          };
          img.onerror = (error) => {
      ...
      if (!slotEntries.length) {
        ...
        alert(
          "Tidak ada foto yang dapat dirender. Pastikan minimal satu foto dipilih."
        );
        return;
      }
      ...
      const loadSlotImage = (dataUrl, slotIndex) =>
        new Promise((resolve) => {
          ...
          img.onerror = (error) => {
            console.error(
              `❌ Slot ${slotIndex + 1}: Failed to load photo`,
              error
            );
            resolve(null);
          };
      ...
      slotImages.forEach(({ slot, slotIndex, img }) => {
        if (!img) {
          console.warn(
            `⚠️ Slot ${slotIndex + 1}: Image missing after load, skipping`
          );
          return;
        }
        ...
        const photoDisplayWidth =
          imgAspectRatio > slotAspectRatio
            ? slotWidth
            : slotHeight * imgAspectRatio;
        const photoDisplayHeight =
          imgAspectRatio > slotAspectRatio
            ? slotWidth / imgAspectRatio
            : slotHeight;
        ...
        const translateX =
          (transform.translateX ?? transform.x ?? 0) * SCALE_RATIO;
        const translateY =
          (transform.translateY ?? transform.y ?? 0) * SCALE_RATIO;
        ...
        renderedCount += 1;
        console.log(
          `✅ Slot ${slotIndex + 1}: Rendered with transform`,
          transform
        );
      });

      console.log(
        `Photos rendering complete: ${renderedCount} photos rendered`
      );
      ...
      if (frameImage) {
        ...
        try {
          ...
          await new Promise((resolve, reject) => {
            frameImgElement.onload = () => {
              console.log(
                `✅ Frame image loaded: ${frameImgElement.width}x${frameImgElement.height}`
              );
      ...
      console.log(
        `🎨 Final rendering complete: ${renderedCount} photos + frame overlay`
      );
      ...
      if (bundleResult.success) {
        confirmationMessageParts.push(
          "Folder ZIP berisi foto dan video telah diunduh."
        );
        if (!bundleResult.hasVideo) {
          confirmationMessageParts.push(
            "Video tidak tersedia di dalam bundle."
          );
        } else if (
          !bundleResult.framedVideoIncluded &&
          bundleResult.rawVideoCount > 0
        ) {
          confirmationMessageParts.push(
            "Video asli dari setiap slot ikut dimasukkan sebagai cadangan."
          );
        } else if (
          bundleResult.framedVideoIncluded &&
          bundleResult.rawVideoCount > 0
          ...
          confirmationMessageParts.push(
            "Selain video utama, rekaman mentah tiap slot juga disertakan."
          );
        }
      } else {
        if (bundleResult.reason === "zip-generation-timeout") {
          confirmationMessageParts.push(
            "Pembuatan ZIP memakan waktu terlalu lama sehingga dibatalkan, foto dan video diunduh terpisah."
          );
        } else {
          confirmationMessageParts.push(
            "Gagal membuat ZIP, file diunduh secara terpisah."
          );
        }
      }
      ...
      if (framedVideoResult && !framedVideoResult.success) {
        confirmationMessageParts.push(
          "Framed video tidak dapat dibuat untuk sesi ini."
        );
      }
          ...
          if (actualPhotoSrc) {
            ...
            await new Promise((resolve) => {
              img.onload = () => {
                ...
                const imgX =
                  slotX +
                  (slotWidth - scaledWidth) / 2 +
                  (transform.translateX || 0);
                const imgY =
                  slotY +
                  (slotHeight - scaledHeight) / 2 +
          ...
          if (photos[i]) {
            ...
            await new Promise((resolve) => {
              img.onload = () => {
                ...
                // Calculate position with transform offset
                const imgX =
                  slotX + (slotWidth - scaledWidth) / 2 + transform.x;
                const imgY =
                  slotY + (slotHeight - scaledHeight) / 2 + transform.y;
          ...
          {useMobileLayout ? (
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  color:
                    isSaving || !hasVideosFromCamera ? "#94a3b8" : "#111827",
                  cursor:
                    isSaving || !hasVideosFromCamera
                      ? "not-allowed"
                      : "pointer",
                  transition: "color 0.2s ease",
                  letterSpacing: "-0.01em",
            ...
            >
              ...
              <button
                ...
                style={{
                  background:
                    isSaving || !hasVideosFromCamera ? "#f5f5f5" : "#fff",
                  border: `2px solid ${
                    isSaving || !hasVideosFromCamera ? "#ccc" : "#E8A889"
                  }`,
            ...
            style={
              isMobile
                ? {
                    ...
                    borderRadius: "999px",
                    background:
                      "linear-gradient(135deg, rgba(250,242,235,0.92) 0%, rgba(242,225,210,0.92) 100%)",
          ...
          <div
            style={{
              background: isMobile
                ...
                : "#fff",
              borderRadius: "20px",
              padding: isMobile ? "clamp(1.1rem, 4.5vw, 1.5rem)" : "1.35rem",
              textAlign: "center",
          ...
          >
            ...
            <div
              style={{
                ...
                borderRadius: "22px",
                padding: isMobile ? "1.75rem 1.3rem" : "2.25rem 1.5rem",
            ...
            >
              {swapModeActive && swapSourceSlot !== null && (
                ...
                >
                  <span>
                    Pilih slot lain untuk menukar dengan Slot{" "}
                    {swapSourceSlot + 1}
                  </span>
                  <button
                    type="button"
              ...
              {frameConfig && frameImage ? (
                ...
                >
                  ...
                  {/* LAYER 1: Background photo (if exists) */}
                    ...
                    <div
                      style={{
                        ...
                        zIndex: Number.isFinite(backgroundPhotoElement.zIndex) 
                          ...
                          : -4000, // Default very low z-index for background
                        overflow: 'hidden',
                        pointerEvents: 'none',
                      }}
                    >
                  ...
                  {/* LAYER 2: Photo Slots - menyesuaikan dengan frame rigid */}
                  {/* ✅ SKIP for custom frames - photo slots already rendered via designer elements */}
                  {!frameConfig.isCustom && frameConfig.slots.map((slot, slotIndex) => {
                    const isSwapSource =
                      swapModeActive && swapSourceSlot === slotIndex;
                    const isSwapCandidate =
                      swapModeActive &&
                      swapSourceSlot !== null &&
                      swapSourceSlot !== slotIndex;
                    const isSelectedForEdit =
                      !swapModeActive && selectedPhotoForEdit === slotIndex;
                    const isActiveSlot = selectedPhotoForEdit === slotIndex;
                    const canPanThisSlot =
                      !swapModeActive && panModeEnabled && isActiveSlot;
                    const isDraggingThisPhoto =
                      canPanThisSlot && isDraggingPhoto;
                    ...
                    const resolvedPhotoIndex =
                      slot.photoIndex !== undefined
                        ? slot.photoIndex
                        : slotIndex;
                    const slotHasCustomPhoto =
                      frameConfig?.duplicatePhotos && slotPhotos[slotIndex];
                    const photoSrc = slotHasCustomPhoto
                      ? slotPhotos[slotIndex]
                      : photos[resolvedPhotoIndex];
                    const slotFilterId = resolveSlotFilterId(slotIndex);
                    const filterCssValue = getFilterCssValue(slotFilterId);
                    const photoStyle = calculatePhotoCropStyle(
                      frameConfig,
                      slotIndex
                    );

                    // ✅ USE ORIGINAL Z-INDEX from slot (not hardcoded)
                    const slotZIndex = Number.isFinite(slot.zIndex) && slot.zIndex >= 0
                      ? slot.zIndex
                      : 100 + slotIndex; // Fallback
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          zIndex: slotZIndex, // ✅ Use original z-index
                          overflow: "hidden",
                          backgroundColor,
                          border: borderStyle,
                      ...
                      >
                        {photoSrc ? (
                          ...
                          >
                            <img
                              src={photoSrc}
                              alt={`Photo ${resolvedPhotoIndex + 1}${
                                slot.photoIndex !== undefined
                                  ? " (duplicate)"
                                  : ""
                              }`}
                              style={{
                                ...
                                cursor: imageCursor,
                                filter:
                                  filterCssValue && filterCssValue !== "none"
                                    ? filterCssValue
                                    : "none",
                  ...
                  {/* LAYER 3: Designer elements (text, shapes, uploads) - rendered on top of photo slots */}
                  {designerElements && designerElements.length > 0 && designerElements.map((element, idx) => {
                    if (!element || !element.type) return null;

                    // ✅ USE ORIGINAL Z-INDEX from element
                    const elemZIndex = Number.isFinite(element.zIndex)
                      ? element.zIndex
                      : 200 + idx; // Fallback to high z-index

                    // Calculate positions for current preview size
                    ...
                    const previewWidth = framePreviewRef.current?.offsetWidth || 280;
                    const previewHeight = framePreviewRef.current?.offsetHeight || (previewWidth * canvasHeight / canvasWidth);
                    ...
                    if (element.type === 'text') {
                      ...
                      return (
                        <div
                          key={`designer-text-${element.id || idx}`}
                          style={{
                            position: 'absolute',
                            left: `${scaledLeft}px`,
                            top: `${scaledTop}px`,
                    ...
                    if (element.type === 'shape') {
                      return (
                        <div
                          key={`designer-shape-${element.id || idx}`}
                          style={{
                            position: 'absolute',
                            left: `${scaledLeft}px`,
                            top: `${scaledTop}px`,
                    ...
                    // Render upload element (image)
                    if (element.type === 'upload' && element.data?.image) {
                      return (
                        <div
                          key={`designer-upload-${element.id || idx}`}
                          style={{
                            position: 'absolute',
                            left: `${scaledLeft}px`,
                            top: `${scaledTop}px`,
                  ...
                  {selectedPhotoForEdit !== null &&
                    !swapModeActive &&
                    frameConfig.slots[selectedPhotoForEdit] &&
                    (() => {
                      const activeSlot =
                        frameConfig.slots[selectedPhotoForEdit];
                      ...
                      const overlayTopPercent =
                        (activeSlot.top + activeSlot.height) * 100;
                      const overlayGap = isMobile ? "0.45rem" : "0.65rem";
                      const transformValue = (
                        photoTransforms[selectedPhotoForEdit]?.scale || 1
                      ).toFixed(1);
                        ...
                        >
                          ...
                          >
                            <button
                              ...
                              style={{
                                background: panModeEnabled
                                  ? "rgba(148, 233, 209, 0.35)"
                                  : "rgba(255, 255, 255, 0.14)",
                                border: panModeEnabled
                                  ? "1px solid rgba(148, 233, 209, 0.65)"
                            ...
                            <button
                              ...
                              style={{
                                ...
                                cursor: "pointer",
                                boxShadow:
                                  "0 6px 16px rgba(232, 168, 137, 0.45)",
                              }}
                            >
                              <img
                                src={swapIcon}
                  ...
                  {hasDevAccess &&
                    ...
                    frameConfig.slots.map((slot, slotIndex) => {
                      const pixels = calculateSlotPixels(
                        frameConfig,
                        slotIndex
                      );
                      return (
                  ...
                  >
                    ...
                    >
                      <div style={{ textAlign: isMobile ? "center" : "left" }}>
                        <strong>Frame:</strong> {frameConfig.name}
                        <br />
                        ...
                        <tbody>
                          {frameConfig.slots.map((slot, index) => {
                            ...
                            const isRatioCorrect =
                              Math.abs(pixels.calculatedRatio - 0.8) < 0.01;
                            return (
                              <tr key={slot.id}>
                                <td
                                  style={{
                                ...
                                >
                                  L:{(slot.left * 100).toFixed(0)}% T:
                                  {(slot.top * 100).toFixed(0)}%<br />
                                  W:{(slot.width * 100).toFixed(0)}% H:
                                  {(slot.height * 100).toFixed(0)}%
                                </td>
                                <td
                                  style={{
                                    border: "1px solid #ddd",
                                ...
                                >
                                  {pixels.calculatedRatio.toFixed(2)}
                                  <br />
                                  {!isRatioCorrect && (
                    ...
                    >
                      ...
                      <br />
                      • Red boxes show slot positions
                      <br />
                      • Aspect ratio should be 0.80 for 4:5
                      <br />• Adjust coordinates in frameConfigs.js
                    </div>
                  </div>
                )
              ) : (
                ...
                >
                          ...
                          >
                            ...
                            <button
                              ...
                              style={{
                                ...
                                fontSize: "0.85rem",
                                fontWeight: "600",
                                cursor: devAuthStatus.loading
                                  ? "wait"
                                  : "pointer",
                                opacity: devAuthStatus.loading ? 0.7 : 1,
                              }}
                            >
                              {devAuthStatus.loading
                                ? "Checking..."
                                : "Unlock Tools"}
                            </button>
                          </form>
                  ...
                  >
                    {FILTER_PRESETS.map((preset) => {
                      ...
                      if (selectedPhotoForEdit !== null) {
                        isSelected =
                          resolveSlotFilterId(selectedPhotoForEdit) ===
                          preset.id;
                        ...
                        <button
                          ...
                          style={{
                            ...
                            boxShadow: isSelected
                              ? "0 5px 14px rgba(232, 168, 137, 0.25)"
                              : "none",
                            transition:
                          ...
                          >
                            <div
                              style={{
                                ...
                                height: "100%",
                                backgroundImage:
                                  "linear-gradient(135deg, #d7d7d7, #ffffff)",
                                filter:
                                  preset.css && preset.css !== "none"
                                    ? preset.css
                                    : "none",
                              }}
                            />
          ...
          {selectedFrame === "Testframe3" && (
            ...
            >
              ...
              <button
                onClick={() => {
                  console.log(
                    `🔍 ${selectedFrame.toUpperCase()} COMPLETE DEBUG:`
                  );
