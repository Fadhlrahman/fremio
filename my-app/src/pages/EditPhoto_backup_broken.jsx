  ...
  <div style={{
    ...
    {(!frameConfig || photos.length === 0) && (
      ...
      <button
        ...
        onClick={() => {
          if (window.confirm('Reset semua data edit photo? Ini akan menghapus frame, foto, dan cache.')) {
            safeStorage.removeItem('frameConfig');
            safeStorage.removeItem('frameConfigTimestamp');
          ...
          <div style={{
            ...
            <button
              onClick={async () => {
                ...
                // Simpan style asli
                const originalWidth = previewCanvas.style.width;
                const originalHeight = previewCanvas.style.height;
                // Set style ke 1080x1920 px
                previewCanvas.style.width = '1080px';
                previewCanvas.style.height = '1920px';
                await html2canvas(previewCanvas, {
                  useCORS: true,
                  backgroundColor: frameConfig?.designer?.canvasBackground || '#fff',
                  imageTimeout: 0
                }).then((canvas) => {
                  // Kembalikan style ke semula
                  previewCanvas.style.width = originalWidth;
                  previewCanvas.style.height = originalHeight;
